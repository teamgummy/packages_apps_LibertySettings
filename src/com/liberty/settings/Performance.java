...
public class Performance extends PreferenceActivity implements OnSharedPreferenceChangeListener {
	...
	private static boolean sendCpu(final String key, final String value, final String fname) {
	...
	static String[] getMHz(int freqs[]){
	...
	static String[] getValues(int freqs[]){
	...
	static int[] getFrequencies(){
		...
		if (freqs != null && freqs != "") {
			...
			CMDProcessor.CommandResult r = cmd.su.runWaitFor("busybox cut -d ' ' -f1 /sys/devices/system/cpu/cpu0/cpufreq/stats/time_in_state");
			freqs = r.stdout;
			if (freqs != null && freqs != "") {
				String[] freqList = freqs.trim().split("[ \n]+");
				SPEED_STEPS = new int[freqList.length];
				...
				SPEED_STEPS =  new int[] { 1000000, 800000, 600000, 300000 };
